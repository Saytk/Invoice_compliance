...
PROMPT_TPL = ROOT / "prompts" / "generate_tests_from_rule.txt"
PROMPTS_OUT = PROJECT_ROOT / "prompts" / "out"
RULES_OUT = PROJECT_ROOT / "rules_out"

RENDERED_PROMPTS = ROOT / "prompts" / "rendered"
...
def build_llm_test_prompt(rule_code: str) -> str:
    """
    Construit le prompt final (string) à envoyer au LLM,
...
def write_rendered_prompt(rule_code: str, *, overwrite: bool = False) -> Path:
    """
    Ecrit le prompt final rendu dans rule_tests/prompts/rendered/<CODE>.final_prompt.txt
    et retourne le path.
    """
    rule_code = rule_code.strip().upper()

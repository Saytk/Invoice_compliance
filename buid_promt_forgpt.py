...
IGNORED_PATHS = {
    "rule_tests/generated",
    "rule_tests/raw_llm_logs",
}
...
def collect_targets(root: Path) -> dict[str, str]:
    ...
    candidate_map = {
        "rules_client": [Path("rules_client.py")],
        "rules_engine": [Path("rules_engine.py")],
        "rules_runtime": [Path("rules_runtime.py")],
        "global_prompt_minimal": [Path("prompts/global_prompt_minimal.txt")],
        "rules_test": [Path("rule_tests")],
        "generate_tests_from_rules": [
            Path("rule_tests/generate_rule_tests.py"),
            Path("rule_tests/prompts/generate_tests_from_rule.txt"),
            Path("rule_tests/build_test_prompt.py"),
        ],
        "llm_generate_rules": [Path("llm_generate_rule.py")],
        "pormptbuilder": [Path("prompts_generator.py")],
    }
